...
async fn get_author_info(repo_path: &str, author_email: &str) -> Option<AuthorInfo> {
    ...
    if parts.len() >= 2 {
        return Some(AuthorInfo {
            login: author_email.split('@').next().unwrap_or(author_email).to_string(),
...
pub fn is_likely_from_china(analysis: &ContributorAnalysis) -> bool {
    ...
    if total_commits > 0 {
        let china_working_hours_commits: usize = analysis.commit_hours
            .iter()
...
pub async fn analyze_repository_contributors(repo_path: &str) -> Vec<ContributorAnalysis> {
    ...
    for email in emails {
        if let Some(analysis) = analyze_contributor_timezone(repo_path, &email).await {
            debug!("分析完成: {} (可能来自中国: {})", 
                   email, 
                   if is_likely_from_china(&analysis) { "是" } else { "否" });
...
pub async fn generate_contributors_report(repo_path: &str) -> ContributorsReport {
    ...
    ContributorsReport {
        ...
        top_china_contributors: china_contributors.iter().take(10).map(|&c| c.clone()).collect(),
        top_non_china_contributors: non_china_contributors.iter().take(10).map(|&c| c.clone()).collect(),
    }
}
...
impl ContributorsReport {
    pub fn print_summary(&self) {
        ...
        info!("中国贡献者: {} 人 ({:.1}%)", self.china_contributors_count, self.china_percentage);
        info!("非中国贡献者: {} 人 ({:.1}%)", self.non_china_contributors_count, 100.0 - self.china_percentage);
        info!("--------------------------------------------------");
        info!("总提交数: {} 次", self.total_commits);
        info!("中国贡献者提交: {} 次 ({:.1}%)", self.china_commits, self.china_commits_percentage);
        info!("非中国贡献者提交: {} 次 ({:.1}%)", self.non_china_commits, 100.0 - self.china_commits_percentage);
        info!("--------------------------------------------------");
        
        if !self.top_china_contributors.is_empty() {
            info!("中国TOP贡献者:");
            for (i, contributor) in self.top_china_contributors.iter().enumerate() {
                let name_display = contributor.name.clone().unwrap_or_else(|| contributor.login.clone());
                info!("  {}. {} - {} 次提交", i+1, name_display, contributor.commits_count);
            }
        }
